"""
Renders each published Post as a static HTML file at
/app/frontend/public/blog/{slug}/index.html
so search engines can index each article as a real URL.
"""
...
# Site constants — kept here so a single file owns the SEO surface.
SITE_TITLE = "Akash Puri"
SITE_TAGLINE = "Programmatic & Display Advertising Specialist"
...
CANONICAL_ORIGIN = ""  # empty → relative URLs. Swap to "https://akashpuri.com" when live.
...
def _slugify(value: str) -> str:
    v = re.sub(r"[^a-z0-9\-\s]", "", (value or "").lower()).strip()
    v = re.sub(r"[\s-]+", "-", v)
    return v or "post"
...
def _first_paragraph(body_md: str) -> str:
    """Return the first non-heading paragraph of body_md as plain text (for meta description)."""
    if not body_md:
        return ""
...
def _related(all_posts: list[dict], current_slug: str, n: int = 2) -> list[dict]:
    """Return up to `n` other published posts, most recent first."""
...
def _tags_html(tags: list[str]) -> str:
    if not tags:
        return ""
    pills = "".join(
        f'<span class="tag-pill">{html.escape(t)}</span>' for t in tags if t
    )
...
def _post_html(post: dict, related: list[dict]) -> str:
    ...
    slug = post.get("slug") or _slugify(title)
    date = post.get("date", "")
    excerpt = post.get("excerpt") or _first_paragraph(post.get("body", ""))[:200]
...
# ---------------------------------------------------------------------------
def write_post_file(post: dict, all_published: list[dict]) -> None:
    """Render + write a single post to /blog/{slug}/index.html.
    If the post isn't published, remove any existing file instead."""
    slug = post.get("slug")
    if not slug:
        return
    dest_dir = BLOG_ROOT / slug
    if not post.get("published"):
        remove_post_file(slug)
        return
    dest_dir.mkdir(parents=True, exist_ok=True)
    related = _related(all_published, slug)
    html_out = _post_html(post, related)
    (dest_dir / "index.html").write_text(html_out, encoding="utf-8")


def remove_post_file(slug: str) -> None:
    if not slug:
        return
    dest_dir = BLOG_ROOT / slug
    if dest_dir.exists():
...
def regenerate_all(published_posts: list[dict]) -> None:
    """Rebuild every /blog/{slug}/ folder from scratch based on the given
    (already-filtered to published) list. Removes stale folders."""
    ...
    for p in published_posts:
        write_post_file(p, published_posts)


def build_sitemap_urls(published_posts: Iterable[dict]) -> list[str]:
    """Return list of /blog/{slug}/ URLs for sitemap generation."""
    return [f"/blog/{p['slug']}/" for p in published_posts if p.get("slug")]
